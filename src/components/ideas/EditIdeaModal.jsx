...
// Helper Function: for idea object initial form values
const getInitialFormData = (idea) => {
  if (!idea) return {
    title: "", shortDescription: "", detailedDescription: "", category: "",
    customCategory: "", tags: "", imageURL: "", estimatedBudget: "",
...
const EditIdeaModal = ({ idea, isOpen, onClose, onUpdated }) => {
  ...
  const [formData, setFormData] = useState(() => getInitialFormData(idea));
  ...
  const handleReset = () => {
    setFormData(getInitialFormData(idea));
  };
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto">
          <div className="p-5 sm:p-7">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              {formData.category === "Other" && (
                <FormField label="Custom Category" required className="md:col-span-2">
                  <InputField
                    name="customCategory"
              ...
              <FormField label="Short Description" required hint="Briefly describe your idea" className="md:col-span-2">
                <TextareaField
          ...
          <div className="border-t border-gray-200 dark:border-zinc-800 p-5 sm:p-7">
            ...
            <div className="space-y-5">
              ...
              <FormField label="Target Audience" required hint="Who will benefit from your idea?">
                <TextareaField
                  name="targetAudience"
...
const FormField = ({ label, required = false, hint = "", children, className = "" }) => (
  <div className={className}>
    <div className="mb-2.5 flex flex-wrap items-center justify-between gap-1.5">
      ...
      {hint && <span className="text-[11px] text-gray-400 dark:text-zinc-500 sm:text-xs">{hint}</span>}
    </div>
    {children}
  </div>
);

const InputField = ({ name, value, onChange, placeholder, type = "text", icon, required = false, min }) => (
  <div className="relative">
    {icon && (
...
const TextareaField = ({ name, value, onChange, placeholder, rows = 4, required = false }) => (
  <textarea
    name={name}
    value={value}
    onChange={onChange}
